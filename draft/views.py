...
def draft_room(request, room_code):
    ...
    if not request.session.get("master", False) and draft.mode == "999":
        messages.info(request, '你没有权限.')
        return render(request, 'draft_entry.html', {
            'draft': draft
        })
    if not request.session.get('authorized_user' + str(draft.id), False):
        ...
        if draft.mode == '1':
            return render(request, 'draft_room_eight.html', {
                'draft': draft,
                'champions': champions,
                'team': team
            })
...
def draft_draft(request, room_code):
    ...
    if request.method == "POST":
        ...
        no = data['no']
        if no:
            # cp_list = [i for i in range(48) if str(i) not in draft.banpick.split('/')]
            # no = cp_list[randrange(0, len(cp_list))]
            # print(f"Choose random no:{no}")
            if draft.banpick:
        ...
        data = {}
        if draft.banpick:
            cp_list = list(range(48))
            for i in draft.banpick.split('/'):
                if i != '999' and int(i) in cp_list:
                    # print(f"i:{i}")
...
def get_timestamp(request):
    return JsonResponse(data=int(datetime.datetime.now().timestamp()), safe=False)