const draft = new Vue({
  ...
  filters: {
    ...
    cpStyle(no) {
      return "backgroundPosition:0px -" + String(Number(no) * 80) + "px;";
    },
  },
  created: function () {
    this.draftRefresh();
  ...
  methods: {
    ...
    draftSelect(no) {
      ...
      if ([6, 7, 8, 9, 10, 11, 16, 17, 18, 19].includes(this.order)) {
        document.querySelector(
          ".od_" + this.order + " .sp"
        ).style.backgroundImage = "url(/assets/img/genshin_all.png)";
        document.querySelector(
          ".od_" + this.order + " .sp"
        ).style.backgroundPosition = "0px -" + String(Number(no) * 80) + "px";
      }
    },
    draftChoice() {
      const self = this;
